...
export function ProductCard({ product, index = 0 }: ProductCardProps) {
  ...
  return (
    <motion.div
      ...
      className="group relative flex flex-col bg-[#0E0E0E] border border-[#1C1C1C] hover:border-[#C6FF00]/50 transition-colors duration-300 rounded-none overflow-hidden"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      <Link href={`/shop/${product.slug}`} className="block relative aspect-[4/5] bg-[#0A0A0A] overflow-hidden">
        ...
        <motion.img
          ...
          animate={shouldReduceMotion ? {} : { scale: isHovered ? 1.02 : 1 }}
        ...
        <div className="absolute top-2.5 left-2.5 flex flex-col gap-1 z-10">
          <span className="px-2 py-0.5 text-[9px] font-mono font-bold tracking-widest uppercase bg-[#C6FF00] text-[#0A0A0A]">
            NOT FOR EVERYONE
          </span>
        ...
        <div className="absolute bottom-0 inset-x-0 p-2.5 bg-gradient-to-t from-[#0A0A0A] via-[#0A0A0A]/90 to-transparent flex items-center justify-between text-[#F5F1E8]">
      ...
      {/* Info Block */}
      <div className="p-3.5 flex flex-col justify-between flex-1 bg-[#0E0E0E] border-t border-[#1C1C1C]">
        <div>
          ...
          <p className="text-[11px] font-mono text-[#8A8A8A] mt-1 line-clamp-1">
            {product.fabricType} • {product.fit}
          </p>
        </div>

        {/* Bottom meta: Color & Available Sizes */}
        <div className="mt-3 pt-2.5 border-t border-[#1C1C1C] flex items-center justify-between text-[10px] font-mono text-[#8A8A8A]">
          <span className="uppercase">{product.color}</span>
          <span className="text-[#C6FF00]/80">S – 4XL</span>
        </div>
      </div>
    </motion.div>
  );
