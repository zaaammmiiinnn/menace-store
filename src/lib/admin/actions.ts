    ...
    try {
      ...
      if (orderObj) {
        ...
        const activeTracking =
          trackingNumber || orderObj.tracking_number || orderObj.trackingNumber || `TRK-MNC-${Date.now().toString(36).toUpperCase()}`;
        ...
        if (customerEmail) {
          ...
          console.log(`[Store Admin Action] Dispatched shipping notification email to ${customerEmail} for order ${orderId}`);
        }
      }
    } catch (emailErr) {
      console.error('[Store Admin Action] Failed to dispatch shipping notification email:', emailErr);
    }
  }
